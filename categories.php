...
<div style="padding:30px;max-width:1100px;margin:auto;">
    ...
    ">
        <?php foreach ($categories as $cat): ?>
            ...
            <?php
            $icon = "📁";
            if ($cat["name"] == "Teknoloji") $icon = "💻";
            elseif ($cat["name"] == "Eğitim") $icon = "📚";
            elseif ($cat["name"] == "İş") $icon = "💼";
            elseif ($cat["name"] == "Sağlık") $icon = "🩺";
            elseif ($cat["name"] == "Sanat") $icon = "🎨";
            ?>

            <a href="ideas.php?category_id=<?= $cat["id"] ?>"
               style="
               background:#fff;
               padding:25px;
               border-radius:12px;
